...
const input = (node, data, config) => {
    ...
    if (node.contract) {
        ...
        findContract(eth, node.contract, txObject, (err, contract) => {
            ...
            if (config.param4) { aArgs.push(helper.resolve(config.param4, data, config.param4)) }
            if (config.param5) { aArgs.push(helper.resolve(config.param5, data, config.param5)) }
